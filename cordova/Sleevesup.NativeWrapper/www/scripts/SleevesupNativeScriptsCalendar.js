...
function ListAllEvents(eStartDate, eEndDate) {
    var startDate = new Date(eStartDate);
    var endDate = new Date(eEndDate);
    window.plugins.calendar.listEventsInRange(startDate, endDate, GetEventList, onError);
}
...
function FindEvent(eTitle, eLocation, eNotes, eStartDate, eEndDate, eStartTime, eEndTime) {
    ...
    window.plugins.calendar.findEvent(title, loc, null, startDate, endDate, FoundEvent);
}
function FoundEvent(data) {
    var i = 0;
    var result = [];
    while (i == 0) {
        result.push(data);
...
function UploadEvents(JsonObject) {
    ...
    oXHR.open('POST', 'http://localhost:59071/Account/PostContact');
